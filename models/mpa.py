"""
DiMP One-Shot: Direct prediction without diffusion framework.

Uses the same DiMP backbone but as a one-shot predictor:
...
class DiMPOneShot(L.LightningModule):
    """
    DiMP as a one-shot predictor (no diffusion).
    ...
    def forward(self, Y):
        ...
        syn = torch.zeros(B, self.K, self.M, device=device)
